...
final class SyntheticComponentsUtil {
     ...
     * @param annotationLiterals to allow generating annotation literal classes
     * @param classOutput to allow generating annotation literal classes
    ...
    static void addParamsFieldAndInit(ClassCreator classCreator, MethodCreator constructor, Map<String, Object> params,
            AnnotationLiteralProcessor annotationLiterals, ClassOutput classOutput, IndexView beanArchiveIndex) {
        ...
        if (params.isEmpty()) {
            ...
            for (Map.Entry<String, Object> entry : params.entrySet()) {
                ...
                if (entry.getValue() instanceof Boolean) {
                    ...
                    for (int i = 0; i < array.length; i++) {
                        constructor.writeArrayValue(valHandle, i, constructor.loadClass(array[i]));
                    }
                } else if (entry.getValue() instanceof ClassInfo) {
                    valHandle = constructor.loadClass(((ClassInfo) entry.getValue()).name().toString());
                } else if (entry.getValue() instanceof ClassInfo[]) {
                    ClassInfo[] array = (ClassInfo[]) entry.getValue();
                    valHandle = constructor.newArray(Class.class, array.length);
                    for (int i = 0; i < array.length; i++) {
                        constructor.writeArrayValue(valHandle, i, constructor.loadClass(array[i].name().toString()));
                    }
                    ...
                    valHandle = annotationLiterals.create(constructor, classOutput, annotationClass,
                            annotationInstance, Types.getPackageName(classCreator.getClassName()));
                } else if (entry.getValue() instanceof AnnotationInstance[]) {
                    ...
                    for (int i = 0; i < array.length; i++) {
                        ...
                        ClassInfo annotationClass = beanArchiveIndex.getClassByName(annotationInstance.name());
                        ResultHandle elementHandle = annotationLiterals.create(constructor, classOutput, annotationClass,
                                annotationInstance, Types.getPackageName(classCreator.getClassName()));
