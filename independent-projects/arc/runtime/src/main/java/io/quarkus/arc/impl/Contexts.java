...
class Contexts {
    ...
    Contexts(ManagedContext requestContext, InjectableContext applicationContext, InjectableContext singletonContext,
        ...
        if (!contexts.isEmpty()) {
            ...
            this.scopes = Set.of(Dependent.class, ApplicationScoped.class, Singleton.class, RequestScoped.class);
        }
    }

    InjectableContext getActiveContext(Class<? extends Annotation> scopeType) {
        // Dependent/Application/Singleton context is always active and it's not possible to register a custom context for these scopes
